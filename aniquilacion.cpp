...
void Nebula::matarPersonas(NodoDoble * raiz, ListaDoble *& eliminadosLista, int idAnterior){
    if (raiz == NULL) return;
    if (estaLista(eliminadosLista,raiz->persona->id)) return;
    else {
        idAnterior = raiz->persona->id;
        if (raiz->persona->vivo == true){
            raiz->persona->vivo = false;
            eliminadosLista->insertarSorted(eliminadosLista->primerNodo,raiz->persona);
        ...
        if (tmp == NULL) return;
        do{
            matarPersonas(tmp,listaEliminados, idAnterior);
            tmp = tmp->siguiente;
...
bool Nebula::estaLista(ListaDoble * eliminados, int idNuevo){
    if (eliminados->isEmpty()) return false;
    else{
        NodoDoble * tmp =  eliminados->primerNodo;
        do{
            if(tmp->persona->id == idNuevo) return true;
            tmp = tmp->siguiente;
        }while(tmp != eliminados->primerNodo);
        return false;
    }
}
...
void EbonyMaw::matarAscendentes(Persona*  personaAnalizada) {
    if (personaAnalizada == NULL) {
        return;
    } else {
        if (!listaIDAniquilados.contains(personaAnalizada->id)) {
            listaIDAniquilados.append(personaAnalizada->id);
            if (personaAnalizada->vivo) {
                mundo->totalMatados++;
                contadorUltimaCorrida++;
                contador++;
                personaAnalizada->vivo = false;
                personaAnalizada->situacion = "Aniquilad@ por Ebony Maw";
                personaAnalizada->cantEliminaciones = personaAnalizada->cantEliminaciones+1;
                mundo->logMuertes->insertarMuerte(personaAnalizada,"Ebony Maw por ser pariente de la persona: " + QString::number(personaAnalizada->id));
            }
            if (personaAnalizada->papa != NULL) {
                matarAscendentes(personaAnalizada->papa);
            }
            if (personaAnalizada->mama != NULL) {
                matarAscendentes(personaAnalizada->mama);
            }
        }
        return;
    }
...
void EbonyMaw::matarDescendentes(Persona* personaAnalizada) {
    if (personaAnalizada == NULL) {
        return;
    } else {
        if (!listaIDAniquilados.contains(personaAnalizada->id)) {
            ...
            if (personaAnalizada->papa != NULL) {
                matarDescendentes(personaAnalizada->papa);
            }
            if (personaAnalizada->mama != NULL) {
                matarDescendentes(personaAnalizada->mama);
            }
        }
        return;
    }
